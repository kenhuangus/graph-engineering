...
def build():
    worker = LlmAgent(
        name="ch11_worker",
        model="stub",
        instruction="Default-deny invoke, bound resume, cut-vertex reachability.",
        tools=[run],
        mode="single_turn",
    )
    return SequentialAgent(name="ch11_adk", sub_agents=[worker], description="Authz is a node in front of the spend tool, not a prompt. Missing edge is deny.")
